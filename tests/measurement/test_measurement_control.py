...
class TestMeasurementControl:  # pylint: disable=too-many-public-methods
    @classmethod
    def setup_class(cls):
    ...
    def test_measurement_control_name(self):
        assert self.meas_ctrl.name == "meas_ctrl"

    def test_measurement_control_save_data(self, parameters):
        self.meas_ctrl.settables(parameters.t)
        self.meas_ctrl.setpoints(np.array([0, 1, 2, 3]))
        self.meas_ctrl.gettables(parameters.sig)

        dataset = self.meas_ctrl.run(save_data=False)
        with pytest.raises(FileNotFoundError):
            _ = dh.locate_experiment_container(dataset.tuid)

        dataset = self.meas_ctrl.run(save_data=True)
        _ = dh.locate_experiment_container(dataset.tuid)
        _ = dh.load_dataset(dataset.tuid)
        _ = dh.load_snapshot(dataset.tuid)

    def test_repr(self, parameters):
        number_points = 5
        ...
        self.meas_ctrl.gettables(parameters.sig)
        repr1 = repr(self.meas_ctrl)
        _ = self.meas_ctrl.run()
        repr2 = repr(self.meas_ctrl)
        expected = (
        ...
        self.meas_ctrl.settables([parameters.t, parameters.amp])
        _ = self.meas_ctrl.run()
        repr2 = repr(self.meas_ctrl)
        expected = (
            "<MeasurementControl: meas_ctrl>\n"
    ...
    def test_iterative_1d(self, parameters):
    ...
    def test_iterative_1d_multi_return(self, parameters):
        xvals = np.linspace(0, 2 * np.pi, 31)

        self.meas_ctrl.settables(parameters.t)
        self.meas_ctrl.setpoints(xvals)
        self.meas_ctrl.gettables(parameters.DualWave())
        dset = self.meas_ctrl.run()
    ...
    def test_soft_averages_iterative_1d(self, parameters):
        def rand():
    ...
    def test_iterative_set_batched_get_1d_raises(self, parameters):
        ...
        self.meas_ctrl.settables(parameters.t)  # iterative settable
        self.meas_ctrl.setpoints(setpoints)
    ...
    def test_iterative_2d_grid(self, parameters):
    ...
    def test_iterative_2d_grid_with_lazy_set(
        self, lazy_set_parameter, lazy_set_argument, lazy_set_turned_on, parameters
    ):
    ...
    def test_iterative_2d_arbitrary(self, parameters):
    ...
    def test_iterative_1d_with_two_settables_signals_plotmon_of_spacing(
        self, setpoints_x, setpoints_y, is_uniformly_spaced, parameters
    ):
        setpoints = np.column_stack((setpoints_x, setpoints_y))

        self.meas_ctrl.settables([parameters.freq, parameters.other_freq])
        self.meas_ctrl.setpoints(setpoints)
        self.meas_ctrl.gettables(parameters.sig)
        dset = self.meas_ctrl.run()
    ...
    def test_iterative_outer_loop_with_inner_batched_2d(self, parameters):
        self.meas_ctrl.settables([parameters.t, parameters.freq])
        ...
        self.meas_ctrl.gettables([parameters.sig, parameters.sig])
    ...
    def test_iterative_3d_grid(self, parameters):
        times = np.linspace(0, 5, 2)
        amps = np.linspace(-1, 1, 3)
        freqs = np.linspace(41000, 82000, 2)

        self.meas_ctrl.settables([parameters.t, parameters.amp, parameters.freq])
    ...
    def test_iterative_3d_multi_return(self, parameters):
        times = np.linspace(0, 5, 2)
        amps = np.linspace(-1, 1, 3)
        freqs = np.linspace(41000, 82000, 2)
        ...
        self.meas_ctrl.settables([parameters.t, parameters.amp, parameters.freq])
        self.meas_ctrl.setpoints_grid([times, amps, freqs])
        self.meas_ctrl.gettables(
            [parameters.DualWave(), parameters.sig, parameters.DualWave()]
        )
        dset = self.meas_ctrl.run()
    ...
    def test_batched_attr_raises(self, parameters):
        ...
        self.meas_ctrl.settables([parameters.freq, parameters.t])
    ...
    def test_batched_grid_mixed(self, parameters):
        times = np.linspace(0, 5, 3)
        ...
        parameters.other_freq.batch_size = (
            3  # odd size and different value for extra test
        )
        ...
        self.meas_ctrl.setpoints_grid(setpoints)
        self.meas_ctrl.gettables(parameters.sig2)
    ...
    def test_progress_callback(self, parameters):
    ...
    def test_meas_ctrl_plotmon_integration(self, parameters):
    ...
    def test_exception_not_silenced_set_get(self, parameters):
        class BadSetter:
            def __init__(self):
                self.name = "spec_freq"
            ...
            def prepare(self):
                # Exception expected
                print(self.non_existing_param)  # pylint: disable=no-member

            def set(self, _):
                pass

        self.meas_ctrl.settables(BadSetter())
        self.meas_ctrl.setpoints(np.linspace(0, 1, 10))
        self.meas_ctrl.gettables(parameters.freq)
        with pytest.raises(
            AttributeError,
        ...
        self.meas_ctrl.gettables(BadGetter())
        with pytest.raises(
            AttributeError,
...
class TestGridSetpoints:
    @classmethod
    def setup_class(cls):
        cls.sp_i0 = np.array([0, 1, 2, 3])
        cls.sp_i1 = np.array([4, 5])
        cls.sp_i2 = np.array([-1, -2])

        base_batched = [1, 2, 6]
        cls.sp_b0 = np.array(base_batched)
        cls.sp_b1 = np.array(base_batched) * 2
        cls.sp_b2 = np.array(base_batched) * 3

        cls.iterative = [cls.sp_i0, cls.sp_i1, cls.sp_i2]
        cls.batched = [cls.sp_b0, cls.sp_b1, cls.sp_b2]

    @classmethod
    def mk_settables(cls, batched_mask):
        settables = []
