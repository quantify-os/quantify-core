...
def test_operation_duration_single_pulse():
    dgp = DRAGPulse(G_amp=.8, D_amp=-.3, phase=24.3, duration=20e-9, freq_mod=15e6, ch_I='ch1', ch_Q='ch2')
...
def test_operation_duration_single_pulse_delayed():
    dgp = DRAGPulse(G_amp=.8, D_amp=-.3, phase=24.3, duration=10e-9, freq_mod=15e6, ch_I='ch1', ch_Q='ch2', t0=3.4e-9)
    assert dgp.duration == pytest.approx(13.4e-9)


def test_operation_add_pulse():
    dgp1 = DRAGPulse(G_amp=.8, D_amp=-.3, phase=0, duration=20e-9, freq_mod=15e6, ch_I='ch1', ch_Q='ch2', t0=0)
    ...
    dgp = DRAGPulse(G_amp=.8, D_amp=-.3, phase=0, duration=20e-9, freq_mod=15e6, ch_I='ch1', ch_Q='ch2', t0=0)
    x90.add_pulse(dgp)
    assert len(x90['pulse_info']) == 1


def test_operation_duration_composite_pulse():
    dgp1 = DRAGPulse(G_amp=.8, D_amp=-.3, phase=24.3, duration=10e-9, freq_mod=15e6, ch_I='ch1', ch_Q='ch2', t0=0)
    assert dgp1.duration == pytest.approx(10e-9)

    # Adding a shorter pulse is not expected to change the duration
    dgp2 = DRAGPulse(G_amp=.8, D_amp=-.3, phase=24.3, duration=7e-9, freq_mod=15e6, ch_I='ch1', ch_Q='ch2', t0=2e-9)
    dgp1.add_pulse(dgp2)
    assert dgp1.duration == pytest.approx(10e-9)

    # adding a longer pulse is expected to change the duration
    dgp3 = DRAGPulse(G_amp=.8, D_amp=-.3, phase=24.3, duration=12e-9,freq_mod=15e6,ch_I='ch3', ch_Q='ch4', t0=3.4e-9)
    dgp1.add_pulse(dgp3)
