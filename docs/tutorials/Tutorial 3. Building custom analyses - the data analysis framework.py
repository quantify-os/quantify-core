...
def extract_quantities_of_interst(fit_result: lmfit.model.ModelResult) -> dict:
    quantities = {
        "amplitude": fit_result.params["amplitude"].value,
        "frequency": fit_result.params["frequency"].value,
    }
...
quantities_of_interst = extract_quantities_of_interst(fit_result=fit_result)
...
class MyCosineAnalysis:
    ...
    def extract_data(self):
        ...
        self.raw_dataset = load_dataset(tuid)

    def run_fitting(self):
        self.fitting_model = lmfit.Model(
            cos_func
        )  # create a fitting model based on a cosine function
        guess = guess_paramaters(self.fitting_model)
        result = model.fit(
            self.raw_dataset.y0.values, x=self.raw_dataset.x0.values, params=guess
        )
        self.fit_result.update({"cosine": result})

    def extract_quantities_of_interst(self):
    ...
    def plot_fit(self, fig: matplotlib.figure.Figure, ax: matplotlib.axes.Axes):
        # plot data
        self.raw_dataset.y0.plot.line(ax=ax, x="x0", marker="o", label="Data")

        # plot fit
        x_fit = np.linspace(self.raw_dataset["x0"][0], self.raw_dataset["x0"][-1], 1000)
        y_fit = cos_func(x=x_fit, **fit_result.best_values)
        ax.plot(x_fit, y_fit, label="Fit")
        ax.legend()
        ...
        # set units-aware tick labels
        set_xlabel(
            ax, self.raw_dataset.x0.long_name, self.raw_dataset.x0.attrs["units"]
        )
        set_ylabel(
            ax, self.raw_dataset.y0.long_name, self.raw_dataset.y0.attrs["units"]
        )
    ...
    def create_figures(self):
        ...
        fig_name = ax_name = "Cosine data and fit"
        self.figs_mpl.update({fig_name: fig})
        # keep a reference to `ax` as well
        # it can be accessed later to apply modifications (e.g., in a notebook)
        self.figs_axs.update({ax_name: ax})
